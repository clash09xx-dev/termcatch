...
import { formatCurrency as fmtMoney, intlLocale } from "@/lib/i18n/format";
...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<{ period?: string }> }) {
  ...
  const D_SHORT = DOW_KEYS.map((k) => dict.weekdays.short[k]);
  const monthShort = (m: number) =>
    new Intl.DateTimeFormat(intlLocale(locale), { month: "short" }).format(new Date(2024, m, 1));
  ...
  if (period === "week") {
    ...
    series = Array.from({ length: now.getMonth() + 1 }, (_, m) => {
      ...
      return { label: monthShort(m), value: v };
    });
  }
  ...
  const busiestDay = dayCount[busiestIdx] > 0 ? T.busiestDays[busiestIdx] : null;
  ...
  const PERIOD_LABEL: Record<Period, string> = { week: T.week, month: T.month, year: T.year };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
      <PageHeader
        title={T.title}
        subtitle={T.subtitle}
        actions={<Segmented ariaLabel={T.periodAria} idBase="an-period" size="sm" value={period} options={periodOpts} />}
      />

      {aiInsights.length > 0 && (
        <div className="fade-rise fade-rise-d1 space-y-2.5">
          <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">{T.aiConclusions}</span>
          <InsightCards insights={aiInsights} severityLabels={dict.insightSeverity} />
        </div>
      )}

      {demand && <DemandHeatmap metrics={demand} t={T} weekdayShort={D_SHORT} />}
      ...
      {total === 0 ? (
        ...
        <>
          ...
          <p className="fade-rise fade-rise-d1 text-[15px] text-slate-600 leading-relaxed">
            {T.summaryPre} <span className="font-semibold text-slate-900 tabular-nums">{money(totalRevenue)}</span> {T.summaryMid} <span className="font-semibold text-slate-900 tabular-nums">{completed.length}</span> {T.summaryPost}{busiestDay && <>, {interpolate(T.summaryBusiest, { day: busiestDay })}</>}.
          </p>

          {/* Focal: revenue chart */}
          <GlassCard className="fade-rise fade-rise-d1 overflow-hidden">
            <CardHeader title={T.revenueOverTime} action={<span className="text-sm font-bold text-slate-900 tabular-nums">{money(totalRevenue)}</span>} />
          ...
          <div className="fade-rise fade-rise-d2 grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard label={T.statCompleted} value={completed.length} sub={interpolate(T.statOfTotal, { n: total })} />
            <StatCard label={T.statAvg} value={money(avgValue)} sub={T.statPerVisit} />
            <StatCard label={T.statNoShow} value={`${noShowRate.toFixed(0)}%`} sub={interpolate(T.statNoShowSub, { n: noShows.length })} />
            <StatCard label={T.statClients} value={cliMap.size} sub={T.statClientsSub} />
          </div>

          <div className="fade-rise fade-rise-d3 grid lg:grid-cols-2 gap-5 items-start">
            {/* Weekday rhythm */}
            <GlassCard className="overflow-hidden">
              <CardHeader title={T.weekRhythm} />
              <div className="p-5"><WeekdayBars data={weekdaySeries} /></div>
            </GlassCard>

            {/* Revenue structure */}
            <GlassCard className="overflow-hidden">
              <CardHeader title={T.revenueStructure} />
              {topServices.length === 0 ? <p className="px-5 py-8 text-center text-sm text-slate-500">{T.noCompleted}</p> : (
                <div className="px-5 py-3">
                  {topServices.map((s, i) => (
                    <div key={s.name} className="py-2.5" style={i > 0 ? { borderTop: HAIRLINE } : undefined}>
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        <p className="text-sm font-semibold text-slate-900 truncate">{s.name}</p>
                        <p className="text-sm font-bold text-slate-900 tabular-nums flex-shrink-0">{money(s.revenue)}</p>
                      </div>
            ...
            <GlassCard className="overflow-hidden lg:col-span-2">
              <CardHeader title={T.topClients} action={<InkLink href="/business/crm" size="sm">{T.allClients}</InkLink>} />
              {topClients.length === 0 ? <p className="px-5 py-8 text-center text-sm text-slate-500">{T.noData}</p> : (
                <div>
                  {topClients.map((c, i) => (
                    <div key={c.name} className="flex items-center gap-3 px-5 py-3" style={i > 0 ? { borderTop: HAIRLINE } : undefined}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 tabular-nums">{c.count} {T.visits}</p>
                      </div>
                      <p className="text-sm font-bold text-slate-900 tabular-nums flex-shrink-0">{money(c.spent)}</p>
                    </div>
                  ))}
                </div>
              )}
