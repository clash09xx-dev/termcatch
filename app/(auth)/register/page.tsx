    ...
    <div>
      ...
      >
        {(["CUSTOMER", "BUSINESS_OWNER"] as const).map((r) => (
          ...
          >
            {r === "CUSTOMER" ? t.auth.customer : t.auth.businessOwner}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-2.5">
        <form action={signInWithGoogleAction}>
          {/* Preserve the selected role + post-auth target (e.g. a booking in
              progress) through the Google OAuth round-trip. */}
      ...
      <p className="mt-2.5 text-[11px] text-gray-400 leading-relaxed">
        {t.auth.oauthTermsPre} {APPLE_SIGNIN_ENABLED ? `Google ${t.auth.or} Apple` : "Google"} {t.auth.oauthTermsMid}{" "}
        <Link href="/terms" target="_blank" className="underline underline-offset-2 hover:text-gray-600">{t.footer.terms}</Link>{" "}
        {t.auth.termsAnd}{" "}
        <Link href="/privacy" target="_blank" className="underline underline-offset-2 hover:text-gray-600">{t.footer.privacy}</Link>.
      </p>
      ...
      <form action={formAction} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="firstName" className="block text-sm font-medium text-gray-700 mb-1.5">{t.auth.firstName}</label>
            <input
          ...
          <div>
            <label htmlFor="lastName" className="block text-sm font-medium text-gray-700 mb-1.5">{t.auth.lastName}</label>
            <input
        ...
        <div>
          ...
          <PasswordInput
            ...
            autoComplete="new-password" required placeholder={t.auth.passwordMin}
            className={inputCls}
          />
        ...
        <div>
          ...
          <PasswordInput
            ...
            autoComplete="new-password" required placeholder={t.auth.confirmPassword}
            className={inputCls}
          />
        ...
        >
          ...
          ) : role === "BUSINESS_OWNER" ? t.nav.registerSalon : t.auth.createAccount}
        </button>
      </form>
    </div>
  );
...
function VerifyEmailStep({
  ...
  const submit = (value: string) => {
    ...
    if (value.length !== OTP_LENGTH) {
      setError(interpolate(t.auth.enterFullCode, { n: OTP_LENGTH }));
      return;
    }
  ...
  const resend = () => {
    ...
    start(async () => {
      ...
      if (res?.error) {
        ...
        setNotice(res?.success ?? t.auth.newCodeSent);
        setCode("");
        setCooldown(60);
      }
    });
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-gray-900 tracking-tight">{t.auth.verifyTitle}</h1>
        <p className="mt-1.5 text-sm text-gray-500 leading-relaxed">
          {interpolate(t.auth.verifyBodyFull, { n: OTP_LENGTH })}{" "}
          <span className="font-medium text-gray-900 break-all">{email}</span>.
        </p>
        <p className="mt-1 text-xs text-gray-400">
          {interpolate(t.auth.signingUpAs, { role: role === "BUSINESS_OWNER" ? t.auth.businessOwner : t.auth.customer })}
        </p>
      </div>
      ...
      <div className="mt-5 text-center text-sm">
        {cooldown > 0 ? (
          <span className="text-gray-400">
            {interpolate(t.auth.resendIn, { n: cooldown })}
          </span>
        ) : (
          <button
            type="button"
