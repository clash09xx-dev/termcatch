...
export default async function EmployeeCalendar({ searchParams }: { searchParams: Promise<{ date?: string }> }) {
  ...
  const label = new Date(start).toLocaleDateString(intlLocale(locale), { timeZone: "Europe/Warsaw", weekday: "long", day: "numeric", month: "long" });

  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <PageHeader title={T.calendar} subtitle={T.yourSchedule} />

      <div className="flex items-center justify-between gap-2">
        <Link href={`/employee/calendar?date=${prev}`} className="rounded-xl px-3 py-2 text-sm font-medium text-slate-600 hover:text-slate-900" style={CHIP}>{T.prevDay}</Link>
        <span className="text-sm font-semibold text-slate-900">{label}</span>
        <Link href={`/employee/calendar?date=${next}`} className="rounded-xl px-3 py-2 text-sm font-medium text-slate-600 hover:text-slate-900" style={CHIP}>{T.nextDay}</Link>
      </div>

      <GlassCard className="overflow-hidden">
        {appts.length === 0 ? (
          <div className="p-6"><EmptyState icon={<Ico />} title={T.noAppointmentsDay} body={T.noAppointmentsDayBody} /></div>
        ) : (
